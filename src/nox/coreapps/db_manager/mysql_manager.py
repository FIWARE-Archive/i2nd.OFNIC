...
# @Updated by Federico Cimorelli <cimorellifederico@gmail.com>
...
class MySQLManager(Component):
    ...
    @check_db_connection    
    def save_staticflow(self, flowID, flow):
        query = "insert into staticflow (id,switch,duration,inport,outport,cookie,nw_src,nw_dst,ip_proto,tp_src,tp_dst,date_time,queue) values('"+str(flowID)+"','"+str(flow['switch'])+"','"+str(flow['duration'])+"','"+str(flow['inport'])+"','"+str(flow['outport'])+"','"+str(flow['cookie'])+"','"+str(flow['nw_src'])+"','"+str(flow['nw_dst'])+"','"+str(flow['ip_proto'])+"','"+str(flow['tp_src'])+"','"+str(flow['tp_dst'])+"',NOW(),"+str(flow['queue']) +");"
        self.cursor.execute(query)
        return True
    
    @check_db_connection
    def remove_staticflow(self, flowID):
        query = "delete from staticflow where id='"+str(flowID)+"';"
        self.cursor.execute(query)

    def load_staticflow(self, switch):
        query = "SELECT * FROM staticflow WHERE switch='"+str(switch)+"';"
        flows = {}
