...
class Component: 
    ...
    def make_action_array(self, actions):
        ...
        for action in actions:
            if action[0] == openflow.OFPAT_OUTPUT \
                    ...
                    and len(action[1]) == 2:
                a = struct.pack("!HHHH", action[0], 8, action[1][1], action[1][0])
            elif action[0] == openflow.OFPAT_ENQUEUE:
                a = struct.pack("!HHHHHHI", action[0], 16, action[1][1], 0,0,0, action[1][2])
                ...
                a = struct.pack("!HHHBH", action[0], 8, action[1], 0, 0)
    ...
    def send_flow_command(self, dp_id, command, attrs, 
                          ...
                          add_args=None,
                          hard_timeout=openflow.OFP_FLOW_PERMANENT, cookie=0):
        m = set_match(attrs)
        if m == None:
            return False
        ...
        self.ctxt.send_flow_command(dp_id, command, m, idle_timeout,
                                    hard_timeout, oactions, buffer_id, priority, cookie)
    ...
    def install_datapath_flow(self, dp_id, attrs, idle_timeout, hard_timeout,
                              actions, cookie=0, buffer_id=None, 
                              priority=openflow.OFP_DEFAULT_PRIORITY,
        ...
        self.send_flow_command(dp_id, openflow.OFPFC_ADD, attrs, priority,
                          (idle_timeout, actions, buffer_id), hard_timeout, cookie)
    ...
    ################################################################################
    # register a handler to be called whenever flow statistics are
    # returned by a switch.
    #
    # handler will be called with the following args:
    #
    # handler(dp_id, stats)
    #
    # Stats is a dictionary of flow stats with the following keys:
    #
    #
    ################################################################################
