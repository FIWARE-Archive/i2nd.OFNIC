...
from nox.netapps.snmp_routing.bod_routing import bod_routing
from nox.netapps.snmp_routing.bod_routing import Netic_path_info

class bod_routing_ws(Component):
    ...
    def install(self):
        ...
        self.bodrt = self.resolve(bod_routing)
        self.discoveryws = self.resolve(discoveryws)
        self.discovery = self.discoveryws.discovery
        
        #register a callback when paths expire
        
        self.bodrt.register_path_expire_cb(self.on_path_expired)
        ...
        # /ws.v1/netic/OFNIC/virtualpath/{id}
    ...
    def _netic_get_path_info(self,request,arg):
        a = {}
        nodesList = []
        pathID = str(arg['{path_ID}'])
        npd = self.bodrt.netic_get_path_info(pathID);
...
#        a['Nodes'] = nodesList
        ...
        a['Source IP'] = npd.nw_src;
        a['Dest IP'] = npd.nw_dst;
        a['Bandwidth'] = npd.bandwidth;
        neticResponse(request,NTC_OK,a)
    
    def _netic_show_paths(self,request,arg):
        a = {}
        a['Paths'] = []
        outString = self.bodrt.netic_get_path_list()
    ...
    def on_path_expired(self, pathID):
        self.discoveryws.remove_existing_path(pathID)

    def getInterface(self):
        return str(bod_routing_ws)


def getFactory():
    class Factory:
        def instance(self, ctxt):
            return bod_routing_ws(ctxt)
    return Factory()
